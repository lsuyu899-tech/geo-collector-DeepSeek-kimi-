...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Entry(key_frame, textvariable=self.moonshot_key, show="*", width=38).grid(row=0, column=1, sticky="w", pady=4)
        ttk.Label(key_frame, text="ARK_API_KEY").grid(row=0, column=2, sticky="w", padx=(16, 8), pady=4)
        ttk.Entry(key_frame, textvariable=self.ark_key, show="*", width=38).grid(row=0, column=3, sticky="w", pady=4)
        ttk.Label(key_frame, text="DEEPSEEK_API_KEY").grid(row=1, column=0, sticky="w", padx=(0, 8), pady=4)
        ttk.Entry(key_frame, textvariable=self.deepseek_key, show="*", width=38).grid(row=1, column=1, sticky="w", pady=4)
    ...
    def _start(self):
        ...
        if self.moonshot_key.get().strip():
            env["MOONSHOT_API_KEY"] = self.moonshot_key.get().strip()
        if self.ark_key.get().strip():
            env["ARK_API_KEY"] = self.ark_key.get().strip()
        if self.deepseek_key.get().strip():
            env["DEEPSEEK_API_KEY"] = self.deepseek_key.get().strip()
